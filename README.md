...
This repo is being built out into a collaborative markdown editor for
GitHub: rich-text multiplayer editing backed by a real git branch/PR, no
...
- [Collaborative Session Prototype](#session-prototype)
  - [What's Implemented](#session-implemented)
  - [How To Test It](#session-testing)
  - [Bugs Found And Fixed](#session-bugs)
  - [Current Scope](#session-scope)
  - [Future Improvements](#session-future)
...
## Collaborative Session Prototype <a name="session-prototype"></a>

P2P markdown-sync prototype built on top of the `hello-pear-electron`
template: an Automerge CRDT doc, replicated peer-to-peer over
Hyperswarm/Corestore, no server. This is a backend/plumbing proof of
concept, not the final product - see [Current Scope](#session-scope) and
[Future Improvements](#session-future) below.

### What's Implemented <a name="session-implemented"></a>

- `workers/session.js` - a Bare worker, separate from `workers/main.js`
  (which still only handles the app's own OTA updater).
  - Opens a Corestore, with one named hypercore per session participant
    (currently just `"self"` - see limitations).
  - Holds an Automerge document with a single `markdown` text field.
  - Joins a Hyperswarm topic; on connecting to a peer, exchanges hypercore
    public keys directly over the raw connection, then replicates via
    `store.replicate(connection)`.
  - Applies incoming peer changes with `Automerge.applyChanges()`, so
    concurrent edits merge via Automerge's text CRDT instead of one side's
    edits clobbering the other's.
  - Deterministically decides which peer creates the session's genesis
    document (see [Bugs Found And Fixed](#session-bugs)).
- `renderer/index.html` / `renderer/app.js` - a plain `<textarea>` bound to
  the session worker, plus a readout of the local hypercore key/length and
  connected peer count. This is a diagnostic UI, not the rich-text JIT
  surface the editor is meant to have eventually.

### How To Test It <a name="session-testing"></a>

**Single instance** (no networking, just local Automerge + Corestore):

```sh
npm start
```

Type in the textarea; the "Synced from Automerge doc" box below it should
update to match.

**Two instances** (real P2P sync): electron-forge's `--` argument passthrough
is unreliable with extra flags on Windows, so start the second instance by
calling the local Electron binary directly instead of via `npm start`:

```sh
npm start
# in a second terminal:
node_modules\.bin\electron.cmd . --storage C:\some\other\path --no-updates
```

Type in either window; the change should appear in the other's synced-doc
box within a couple of seconds, and each window's peer count should read 1.

### Bugs Found And Fixed <a name="session-bugs"></a>

- **Automerge doesn't load under Bare by default.** Its default build picks
  the `"node"` exports condition, which hard-requires Node's real `fs` to
  load its wasm binary - Bare has no `fs`. Fixed by requiring the base64
  wasm build by relative path instead, plus polyfilling `TextEncoder`/
  `TextDecoder` (`bare-encoding/global`) and `crypto` (`bare-crypto/global`),
  neither of which Bare provides globally.
- **Windows Firewall silently blocked `bare.exe`.** An earlier run triggered
  a firewall consent prompt with nobody to answer it, which Windows resolved
  by creating a persistent inbound Block rule - allowing outbound DHT
  bootstrap traffic to keep working (so things looked like they were
  connecting) while actually preventing any peer from ever reaching us.
  If two instances aren't finding each other, check
  `Get-NetFirewallRule -DisplayName "bare.exe"` for stray Block entries.
- **Genesis bootstrap race.** Two peers starting at nearly the same instant
  could each independently call `Automerge.init()` and create their own
  `markdown` field. Automerge treats that as a whole-object conflict, not a
  text merge - one side's entire edit history silently loses, permanently.
  Fixed by having peers compare hypercore public keys once connected and
  deterministically agree on exactly one genesis creator, instead of racing
  a timer.

### Current Scope <a name="session-scope"></a>

- The Hyperswarm topic is a **hardcoded placeholder constant** - every
  instance of the app finds every other instance. There's no real
  session/invite identity yet.
- No GitHub integration at all yet (no fetch, no auth, no PR creation).
- The editor is a plain `<textarea>`, not the rich-text JIT-rendered surface
  the final editor is meant to have.
- Only one participant's document field (`markdown`) exists; there's no
  concept of file identity, multiple documents, or reconnecting to a
  specific past session.
- The genesis race fix narrows the failure window but doesn't eliminate it
  entirely: if a peer takes longer than the 2-second grace period to
  connect, both sides could still independently self-initialize. A real
  invite/session-start protocol would remove this edge case entirely.

### Future Improvements <a name="session-future"></a>

- **Checkpoint rollover** (not yet built): periodically call
  `Automerge.save()` for the compacted, columnar-compressed encoding, then
  roll each participant to a fresh hypercore seeded with that blob as block
  0, retiring the prior core. Without this, each participant's hypercore
  grows by one raw (uncompacted) block per keystroke-level change forever.
  Needs a small coordination protocol so peers agree on a quiescent point to
  snapshot from before rolling over, so no concurrent edits are lost between
  snapshot and rollover.
- Real session/topic derivation tied to an actual GitHub file + invite flow.
- Rich-text JIT rendering in place of the plain textarea.
- GitHub device-flow auth and branch/commit/PR creation on session end.
- Autobase-backed session membership/invites, kept separate from document
  merge (which stays Automerge-only).
